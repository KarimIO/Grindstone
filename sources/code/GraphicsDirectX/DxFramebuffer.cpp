...
DxFramebuffer::DxFramebuffer(ID3D11Device *device, ID3D11DeviceContext *deviceContext, FramebufferCreateInfo ci) {
	...
	m_numRenderTargets = ci.numColorTargets;
	...
	for (uint32_t i = 0; i < m_numRenderTargets; i++) {
		...
		textureDesc.Width = ci.width;
		textureDesc.Height = ci.height;
	...
	if (ci.depthFormat != FORMAT_DEPTH_NONE) {
		...
		depthBufferDesc.Width = ci.width;
		depthBufferDesc.Height = ci.height;
		...
		if (FAILED(result)) {
			std::cout << "Failed to create depth SRV for framebuffer\n";
			return;
		}
	}
}
...
void DxFramebuffer::Blit(int i, int x, int y, int w, int h) {
}
...
void DxFramebuffer::BindTextures() {
	std::vector<ID3D11ShaderResourceView*>		textures;
