...
LRESULT CALLBACK Win32Window::WndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam) {
	...
	switch (msg) {
		...
		case WM_CREATE:
			...
			break;
	case WM_CLOSE:
		if (MessageBox(NULL, _T("Are you sure you want to close?"),
			_T("Error!"), MB_ICONEXCLAMATION | MB_YESNO)) {
			should_close_ = true;
			DestroyWindow(hwnd);
		}
		break;
...
bool Win32Window::initialize(WindowCreateInfo create_info) {
	...
	wc.lpszClassName = create_info.title; //title_w.data();
	...
	window_handle_ = CreateWindowEx(ex_style_,
		create_info.title, //title_w.data(),
		create_info.title, //title_w.data(),
		style_,
		window_size_.left,
		window_size_.top,
...
void Win32Window::setInputInterface(InputInterface* ii) {
	input_ = ii;
}
...
void Win32Window::setFullscreen(WindowFullscreenMode mode) {
