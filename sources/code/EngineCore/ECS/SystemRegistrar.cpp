...
void SystemRegistrar::RegisterSystem(const char* name, SystemFactory factory) {
	...
	if (sys != editorSystemFactories.end()) {
		GPRINT_ERROR(LogSource::EngineCore, "Registering a system that has already been registered: {}", name);
		return;
	}
...
void SystemRegistrar::RegisterEditorSystem(const char* name, SystemFactory factory) {
	auto sys = editorSystemFactories.find(name);
	if (sys != editorSystemFactories.end()) {
		GPRINT_ERROR(LogSource::EngineCore, "Registering an editor system that has already been registered: {}", name);
		return;
	}
...
void SystemRegistrar::UnregisterSystem(const char* name) {
	...
	if (sys == systemFactories.end()) {
		GPRINT_ERROR(LogSource::EngineCore, "Unregistering a system that isn't registered: {}", name);
		return;
	}
...
void SystemRegistrar::UnregisterEditorSystem(const char* name) {
	auto sys = editorSystemFactories.find(name);
	if (sys == editorSystemFactories.end()) {
		GPRINT_ERROR(LogSource::EngineCore, "Unregistering an editor system that isn't registered: {}", name);
		return;
	}
