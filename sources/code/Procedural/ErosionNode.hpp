...
public:
	...
	void setSedimentCapacityFactor(float capacity);
	void setMinSedimentCapacity(float capacity);
	void setErodeSpeed(float speed);
	void setDepositSpeed(float speed);
	void setEvaporateSpeed(float speed);
	void setGravity(float gravity);
	void setMaxDropletLifetime(int life);
	void setInitialWaterVolume(float water);
	void setInitialSpeed(float speed);

	void setMap(double*, unsigned int width, unsigned int height);
...
private:
	...
	int *erosionBrushIndicesLengths;
	float **erosionBrushWeights;

	void initializeBrushIndices();
	void erode(int numIterations = 1);
	...
	HeightAndGradient calculateHeightAndGradient(double *nodes, int mapSize, float posX, float posY);
