...
namespace Grindstone {
	namespace GraphicsAPI {
		VulkanRenderPass::VulkanRenderPass(RenderPassCreateInfo ci) : width_(ci.m_width), height_(ci.m_height) {
			...
			for (uint32_t i = 0; i < ci.m_colorClearCount; ++i) {
				...
				uint8_t channels;
				colorAttachment.format = TranslateColorFormatToVulkan(ci.m_colorFormats[i], channels);
			...
			if (ci.m_depthFormat != DepthFormat::None) {
				...
				depthAttachment.format = TranslateDepthFormatToVulkan(ci.m_depthFormat);
