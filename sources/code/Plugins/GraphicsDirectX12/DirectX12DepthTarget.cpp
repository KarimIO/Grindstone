...
namespace Grindstone {
	namespace GraphicsAPI {
		DirectX12DepthTarget::DirectX12DepthTarget(DepthTargetCreateInfo ci) {
		...
		DirectX12DepthTarget::~DirectX12DepthTarget() {
		...
		VkImageView DirectX12DepthTarget::getImageView() {
			return image_view_;
		}
		void DirectX12DepthTarget::BindFace(int k) {
			std::cout << "DirectX12DepthTarget::BindFace is not used.\n";
			assert(false);
		}
	}
}
