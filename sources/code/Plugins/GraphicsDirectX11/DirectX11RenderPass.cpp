...
namespace Grindstone {
	namespace GraphicsAPI {
		DirectX11RenderPass::DirectX11RenderPass(RenderPassCreateInfo ci) : width_(ci.m_width), height_(ci.m_height) {
			...
			for (uint32_t i = 0; i < ci.m_colorClearCount; ++i) {
				...
				uint8_t channels;
				colorAttachment.format = TranslateColorFormatToDirectX11(ci.m_colorFormats[i], channels);
			...
			if (ci.m_depthFormat != DepthFormat::None) {
				...
				depthAttachment.format = TranslateDepthFormatToDirectX11(ci.m_depthFormat);
			...
			if (vkCreateRenderPass(DirectX11GraphicsWrapper::get().getDevice(), &renderPassInfo, nullptr, &render_pass_) != VK_SUCCESS) {
				throw std::runtime_error("failed to create render pass!");
			}
		}

		VkRenderPass DirectX11RenderPass::getRenderPassHandle() {
			return render_pass_;
		}

		uint32_t DirectX11RenderPass::getWidth() {
			return width_;
		}

		uint32_t DirectX11RenderPass::getHeight() {
			return height_;
		}
	}
}
