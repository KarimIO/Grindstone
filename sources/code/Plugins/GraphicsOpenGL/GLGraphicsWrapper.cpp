...
namespace Grindstone {
	namespace GraphicsAPI {
		bool GLCore::initialize(CreateInfo ci) {
		...
		// Creators
		//==================================
		TextureBinding* GLCore::createTextureBinding(TextureBindingCreateInfo createInfo) {
			return static_cast<TextureBinding*>(new GLTextureBinding(createInfo));
		}

		TextureBindingLayout* GLCore::createTextureBindingLayout(TextureBindingLayoutCreateInfo createInfo) {
			return static_cast<TextureBindingLayout*>(new GLTextureBindingLayout(createInfo));
		}

		Framebuffer* GLCore::createFramebuffer(FramebufferCreateInfo ci) {
			return static_cast<Framebuffer*>(new GLFramebuffer(ci));
		}

		RenderPass* GLCore::createRenderPass(RenderPassCreateInfo ci) {
			return 0;
		}

		Pipeline* GLCore::createPipeline(PipelineCreateInfo ci) {
			return static_cast<Pipeline*>(new GLPipeline(ci));
		}

		VertexArrayObject* GLCore::createVertexArrayObject(VertexArrayObjectCreateInfo ci) {
			return static_cast<VertexArrayObject*>(new GLVertexArrayObject(ci));
		}

		CommandBuffer* GLCore::createCommandBuffer(CommandBufferCreateInfo ci) {
			return 0;
		}

		VertexBuffer* GLCore::createVertexBuffer(VertexBufferCreateInfo ci) {
			return static_cast<VertexBuffer*>(new GLVertexBuffer(ci));
		}

		IndexBuffer* GLCore::createIndexBuffer(IndexBufferCreateInfo ci) {
			return static_cast<IndexBuffer*>(new GLIndexBuffer(ci));
		}

		UniformBuffer* GLCore::createUniformBuffer(UniformBufferCreateInfo ci) {
			return static_cast<UniformBuffer*>(new GLUniformBuffer(ci));
		}

		UniformBufferBinding* GLCore::createUniformBufferBinding(UniformBufferBindingCreateInfo ci) {
			return static_cast<UniformBufferBinding*>(new GLUniformBufferBinding(ci));
		}

		Texture* GLCore::createCubemap(CubemapCreateInfo ci) {
			return static_cast<Texture*>(new GLTexture(ci));
		}

		Texture* GLCore::createTexture(TextureCreateInfo ci) {
			return static_cast<Texture*>(new GLTexture(ci));
		}

		RenderTarget* GLCore::createRenderTarget(RenderTargetCreateInfo* rt, uint32_t rc, bool cube) {
			return static_cast<RenderTarget*>(new GLRenderTarget(rt, rc, cube));
		}

		DepthTarget* GLCore::createDepthTarget(DepthTargetCreateInfo rt) {
