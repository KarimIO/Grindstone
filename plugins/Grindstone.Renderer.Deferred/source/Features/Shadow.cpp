...
void Grindstone::Renderer::Shadow::Initialize() {
	// Static so that we only create one CVAR, not one per camera/RenderPass.
	static bool isFreezeCvarCreated = false;
	if (!isFreezeCvarCreated) {
		isFreezeCvarCreated = true;
		Grindstone::CvarSystem* cvarSystem = Grindstone::CvarSystem::GetInstance();
		cvarSystem->CreateBooleanCvar("render.lights.freezeCsm", "Freeze Cascaded Shadow Map cascade generation.", false, false);
	}
}

void Grindstone::Renderer::Shadow::Bind(
	Grindstone::Renderer::RenderGraphBuilder& renderGraphBuilder,
	Grindstone::Renderer::RenderFrameContext& context
) {
	for (const RenderFrameViewContext& view : context.views) {
		const glm::vec3& eyePos = view.eyePos;
		const glm::mat4& cameraProjectionMatrix = view.projectionMatrix;
		const glm::mat4& cameraViewMatrix = view.viewMatrix;
		Grindstone::WorldContextSet& worldContextSet = context.worldContextSet;
		...
		Renderer::RenderGraphBuilderResourceRef shadowAtlasRef = renderGraphBuilder.AddImage(attachmentShadowDepthStencil, Renderer::invalidPassId);
		...
		spotLightView.each(
			[this, &renderGraphBuilder, &shadowAtlasRef, scene, pushRenderingStatsCallback](const entt::entity entityHandle, const Grindstone::TagComponent& tag, Grindstone::SpotLightComponent& spotLightComponent) {
				...
				if (GetAtlasRenderArea(renderArea)) {
					...
					shadowAtlasRef = AddSpotShadowPass(renderGraphBuilder, tag.tag, shadowAtlasRef, metaRect, entity, spotLightComponent, pushRenderingStatsCallback);
				}
			}
		);

		Grindstone::CvarSystem* cvarSystem = Grindstone::CvarSystem::GetInstance();
		bool isFrozen = cvarSystem->GetBoolCvar(cvarSystem->GetCvar("render.lights.freezeCsm"_hash)->arrayIndex);
		if (!isFrozen) {
			...
			directionalLightView.each(
				[this, &renderGraphBuilder, &shadowAtlasRef, &cameraFrustumCorners, &cameraProjectionMatrix, &cameraViewMatrix, eyePos, scene, nearDistance, farDistance](
					const entt::entity entityHandle,
					const Grindstone::TagComponent& tag,
					DirectionalLightComponent& directionalLightComponent
				) {
					directionalLightComponent.eyePosForShadowing = eyePos;
					directionalLightComponent.debugCameraProjectionMatrix = cameraProjectionMatrix;
					directionalLightComponent.debugCameraViewMatrix = cameraViewMatrix;
					const ECS::Entity entity = ECS::Entity(entityHandle, scene);
					...
					for (size_t cascadeIndex = 0; cascadeIndex <= cascadeCount; ++cascadeIndex) {
						cascadeDistances[cascadeIndex] = GetCascadePlane(cascadeIndex, cascadeCount - 1);
					}
		...
		directionalLightView.each(
			[this, &renderGraphBuilder, &shadowAtlasRef, scene, pushRenderingStatsCallback](
				const entt::entity entityHandle,
				const Grindstone::TagComponent& tag,
				DirectionalLightComponent& directionalLightComponent
			) {
				...
				Grindstone::Math::IntRect2D renderArea;
				// TODO: should freeze cascadeCount
				for (size_t cascadeIndex = 0; cascadeIndex < directionalLightComponent.cascadeCount; ++cascadeIndex) {
					if (GetAtlasRenderArea(renderArea)) {
						Grindstone::Renderer::MetaRect metaRect = MetaRect::Pixels(
							static_cast<uint32_t>(renderArea.offset.x),
							static_cast<uint32_t>(renderArea.offset.y),
						...
						float shadowAtlasResolutionF = static_cast<float>(shadowAtlasResolution);
						directionalLightComponent.shadowRenderArea[cascadeIndex] = Math::Rect2D(
							static_cast<float>(renderArea.offset.x) / shadowAtlasResolutionF,
							static_cast<float>(renderArea.offset.y) / shadowAtlasResolutionF,
							static_cast<float>(renderArea.extent.x) / shadowAtlasResolutionF,
							static_cast<float>(renderArea.extent.y) / shadowAtlasResolutionF
						...
						shadowAtlasRef = AddDirectionalShadowPass(renderGraphBuilder, tag.tag, shadowAtlasRef, metaRect, entity, directionalLightComponent, pushRenderingStatsCallback, lightProjectionMatrix, lightViewMatrix, cascadeIndex);
					}
				}
			}
		);
		...
		pointLightView.each(
			[this, &renderGraphBuilder, &shadowAtlasRef, scene, pushRenderingStatsCallback](const entt::entity entityHandle, const Grindstone::TagComponent& tag, Grindstone::PointLightComponent& pointLightComponent) {
				const ECS::Entity entity = ECS::Entity(entityHandle, scene);
				Grindstone::Math::IntRect2D renderArea;
				for (size_t i = 0; i < 6; ++i) {
					if (GetAtlasRenderArea(renderArea)) {
						...
						shadowAtlasRef = AddPointShadowPass(renderGraphBuilder, tag.tag, shadowAtlasRef, metaRect, entity, pointLightComponent, pushRenderingStatsCallback, i);
					}
				}
			}
		);

		context.blackboard.SetValue<Grindstone::Renderer::RenderGraphBuilderResourceRef>("ShadowAtlasRef", shadowAtlasRef);
	}
}

void Grindstone::Renderer::Shadow::PrepareAtlas(uint32_t totalShadowMapCount) {
	currentAtlasIndex = 0;
...
bool Grindstone::Renderer::Shadow::GetAtlasRenderArea(Grindstone::Math::IntRect2D& rect) {
