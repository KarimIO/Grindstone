...
void Grindstone::Renderer::Bloom::Initialize() {
	...
	Grindstone::GraphicsAPI::Sampler::CreateInfo screenSamplerCreateInfo{
		.debugName = "Bloom Screen Sampler",
		.options = {
...
static void InitializeImageSetIfNecessary(
	Grindstone::Math::Uint2 size,
	std::array<Grindstone::Math::Uint2, Grindstone::Renderer::Bloom::BLOOM_MIPS + 1>& mipSizes,
	Grindstone::Renderer::Bloom::ImageSet& set
) {
	if (size == set.size) {
		return;
	}

	auto n = Grindstone::Renderer::Bloom::BLOOM_MIPS;
...
void Grindstone::Renderer::Bloom::Bind(
	Grindstone::Renderer::RenderGraphBuilder& renderGraphBuilder,
	...
	stageOutputs[0] = outRef = FirstDownscale(mipSizes[1], *(ds++), screenSampler, bloomPipeline, pipelineLayout, renderGraphBuilder, litImageRef);
