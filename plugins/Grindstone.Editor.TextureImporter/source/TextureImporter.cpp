...
private:
	...
	uint8_t CombinePixels(const uint8_t* pixelSrc, uint64_t index, uint64_t pitch) const;
	std::vector<uint8_t> CreateMip(const std::vector<uint8_t>& pixel, uint32_t width, uint32_t height);
	void ExtractBlock(
		const uint8_t* inPtr,
	...
	void Convert();
	void ExtractFirstFace(uint8_t faceIndex, std::vector<uint8_t>& output);
	void GenerateMipList(uint8_t faceIndex, uint32_t minMipLevel, std::vector<std::vector<uint8_t>>& uncompressedMips);
	void OutputDds(const std::vector<uint8_t>& outData);
...
uint8_t TextureImporter::CombinePixels(const uint8_t* pixelSrc, uint64_t index, uint64_t pitch) const {
...
std::vector<uint8_t> TextureImporter::CreateMip(const std::vector<uint8_t>& pixel, uint32_t width, uint32_t height) {
...
void TextureImporter::Convert() {
	...
	std::vector<uint8_t> outData(outputTotalSize);
	int minMipLevel = std::max(mipMapCount, 1);
	for (int faceIterator = 0; faceIterator < faceCount; faceIterator++) {
		GenerateFace(minMipLevel, faceIterator, blockSize, outData.data() + (outputFaceSize * faceIterator));
	}
...
void TextureImporter::ExtractFirstFace(uint8_t faceIndex, std::vector<uint8_t>& output) {
	...
	for (uint64_t targetRow = 0; targetRow < texHeight; targetRow++) {
		for (uint64_t targetCol = 0; targetCol < texWidth; targetCol++) {
			...
			for (uint64_t channelIndex = 0; channelIndex < texChannels; ++channelIndex) {
				output[dst++] = CombinePixels(firstPixel, sourceIndex + channelIndex, sourcePitch);
			}
		}
	}
}

void TextureImporter::GenerateMipList(uint8_t faceIndex, uint32_t minMipLevel, std::vector<std::vector<uint8_t>>& uncompressedMips) {
	...
	if (isSixSidedCubemap) {
		ExtractFirstFace(faceIndex, uncompressedMips[0]);
	}
	else {
		size_t firstMipSize = static_cast<size_t>(texWidth) * texHeight * texChannels;
		uncompressedMips[0] = std::vector<uint8_t>(sourcePixels, sourcePixels + firstMipSize);
	}
...
void TextureImporter::GenerateFace(uint32_t minMipLevel, uint32_t faceIterator, uint32_t blockSize, uint8_t* outData) {
	...
	std::vector<std::vector<uint8_t>> uncompressedMips;
	...
	for (uint32_t mipLevelIterator = 0; mipLevelIterator < minMipLevel; mipLevelIterator++) {
		uint8_t* mipSource = uncompressedMips[mipLevelIterator].data();
...
void TextureImporter::OutputDds(const std::vector<uint8_t>& outData) {
	DDSHeader outHeader;
