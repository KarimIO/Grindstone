...
#include <ScriptCSharp/include/ScriptClass.hpp>

#include <ScriptCSharp/include/CSharpManager.hpp>
...
void CSharpManager::SetupComponent(Grindstone::WorldContextSet& cxtSet, entt::entity entity, ScriptComponent& component) {
	...
	if (it == assemblies.end()) {
		if (LoadAssemblyIntoMap(component.assembly.c_str())) {
...
void CSharpManager::PerformReload() {
	if (csharpGlobals.CreateObject == nullptr || csharpGlobals.DestroyObject == nullptr) {
		return;
	}
