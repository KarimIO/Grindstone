...
float bitfieldReverseGS(uint bits) {
...
vec2 Hammersley(uint i, uint N) {
	return vec2( float(i) / float(N), float(bitfieldReverseGS(i)) * 2.3283064365386963e-10 );
}
...
vec3 radiance(vec3 N, vec3 V, vec4 Specular) {
	...
	for (uint i = 0u; i < NumSamples; ++i) {
		...
		vec3 F_ = clamp(Light_F(f0, f90, VoH), 0, 1); // Defined elsewhere as subroutine
		float G_ = clamp(Light_V( NoL, NoV, alphaSqr ), 0, 1); // Defined elsewhere as subroutine
